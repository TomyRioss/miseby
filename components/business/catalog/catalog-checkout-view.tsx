...
/** Checkout del pedido del catálogo multirubro: carrito → datos → envío por WhatsApp. Sin DB. */
export function CatalogCheckoutView({
  ...
  function confirm() {
    ...
    const msg = [
      ...
      `Entrega: ${fulfillment === "retiro" ? "Retiro en el local" : "Delivery"}`,
    ...
    try {
      if (whatsapp) {
        ...
        navigator.clipboard.writeText(msg).then(
          () => toast.success("Pedido copiado. Pasáselo al negocio."),
          () => toast.success("Anotá tu pedido y pasáselo al negocio."),
        );
      }
    } catch (e) {
