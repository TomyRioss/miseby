                ...
                {variants.map((v, i) => (
                  <div key={v.key} className="rounded-xl border border-border p-3">
                    <div className="flex items-center gap-2">
                      ...
                      <button type="button" onClick={() => setVariants((p) => p.filter((x) => x.key !== v.key))} aria-label="Quitar variante" className="rounded-lg p-2 text-muted-foreground hover:bg-muted hover:text-destructive"><Trash2 className="h-4 w-4" /></button>
                    </div>
              ...
              {groups.map((g) => (
                <div key={g.key} className="rounded-xl border border-border p-3">
                  <div className="flex items-center gap-2">
                    ...
                    <button type="button" onClick={() => setGroups((p) => p.filter((x) => x.key !== g.key))} aria-label="Quitar grupo" className="rounded-lg p-2 text-muted-foreground hover:bg-muted hover:text-destructive"><Trash2 className="h-4 w-4" /></button>
                  </div>
                  ...
                  <div className="mt-2 space-y-1.5">
                    {g.modifiers.map((m) => (
                      <div key={m.key} className="flex items-center gap-2">
                        ...
                        <button type="button" onClick={() => setGroups((p) => p.map((x) => x.key === g.key ? { ...x, modifiers: x.modifiers.filter((mm) => mm.key !== m.key) } : x))} aria-label="Quitar opción" className="rounded-lg p-2 text-muted-foreground hover:bg-muted hover:text-destructive"><Trash2 className="h-4 w-4" /></button>
                      </div>
                    ))}
